...
resource "coder_agent" "main" {
  ...
  startup_script         = <<-EOT
    ...
    # install filebrowser for Coder window
    ...
    export KASM_USER="${local.username}"
    ...
    # start kasm server
    /dockerstartup/kasm_default_profile.sh #> /dockerstartup/kasm.log 
    /dockerstartup/vnc_startup.sh #> /dockerstartup/startup.log
...
resource "docker_container" "workspace" {
  ...
  shm_size = 4096  # Set shared memory size to 4GB
  ...
  volumes {
    container_path = "/data/local720/services"
    host_path = "/home/narc/services"
  }

  volumes {
    container_path = "/data/local720/fmriprep"
    host_path = "/mnt/minio/hdd8tb/disk2/ntfs/fmriprep"
  }

  volumes {
    container_path = "/data/local720/services-disk2"
    host_path = "/mnt/minio/hdd8tb/disk2/ntfs/services"
  }
