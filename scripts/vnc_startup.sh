...
# echo Setting up kasm user kasm-user
...
# echo "Stopping all Kasm services..."
# for pid in ${KASM_PROCS[@]}; do
#     kill $pid -9 || echo "Failed to stop process $pid"
# done
...
sudo openssl req -x509 -nodes -days 3650 -newkey rsa:2048 -keyout ${HOME}/.vnc/self.pem -out ${HOME}/.vnc/self.pem -subj "/C=US/ST=VA/L=None/O=None/OU=DoFu/CN=kasm/emailAddress=none@none.none"
...
sudo KASM_USER=$KASM_USER bash $STARTUPDIR/custom_startup.sh >> $STARTUPDIR/custom.log
			...
			case $process in
			  ...
			  kasm_gamepad)
				...
				custom_script)
					...
					custom_startup >> $STARTUPDIR/custom.log
					;;
				*)
